...
namespace BridgeDetectSystem.service
{
    public class ConfigManager
    {
        ...
        private static Dictionary<ConfigKeys, ConfigItem> GetInitialConfig()
        {
            ...
            {
                 //配置项                                                    min  max  default
                {ConfigKeys.basket_upDisLimit,new ConfigItem("挂篮上升位移", 0, 1000, 100 ) },
                {ConfigKeys.basket_downDisLimit,new ConfigItem("挂篮下降位移", 0, 1000, 100 ) },
                ...
                {ConfigKeys.anchor_ForceDiffLimit,new ConfigItem("锚杆力差值上限", 0, 1000, 100 ) },
                {ConfigKeys.frontPivot_DisLimit,new ConfigItem("前支点位移上限", 0, 10, 5 ) },
